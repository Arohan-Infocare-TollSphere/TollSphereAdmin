...
namespace Arohan.TollSphere.Server.UI.Hubs;
...
public sealed class HubClient : IAsyncDisposable
{
    ...
    public async Task<List<Arohan.TollSphere.Application.Common.Interfaces.Identity.UserContext>> GetOnlineUsersAsync(CancellationToken cancellationToken = default)
    {
        var users = await _hubConnection.InvokeAsync<List<Arohan.TollSphere.Application.Common.Interfaces.Identity.UserContext>>(nameof(ISignalRHub.GetOnlineUsers), cancellationToken).ConfigureAwait(false);
        return users ?? new List<Arohan.TollSphere.Application.Common.Interfaces.Identity.UserContext>();
    }
}
